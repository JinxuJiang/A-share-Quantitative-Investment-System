...
from run_storage import validate_run_id, write_json


def main() -> int:
    parser = argparse.ArgumentParser(description="第四层回测验收")
    parser.add_argument("--config", default=str(LAYER_DIR / "config.yaml"))
    parser.add_argument("--run-id", required=True, help="要验收的版本化回测ID")
    args = parser.parse_args()
    run_id = validate_run_id(args.run_id)
    base_config = load_yaml(Path(args.config))
    base_report_root = resolve_layer_path(base_config["output"]["reports_dir"])
    report_root = base_report_root / run_id
    manifest_path = report_root / "run_manifest.json"
    snapshot_path = report_root / "config_snapshot.yaml"
    if not manifest_path.exists() or not snapshot_path.exists():
        raise FileNotFoundError(f"run-id does not contain a complete manifest/config snapshot: {run_id}")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    config = load_yaml(snapshot_path)
    requested_models = list(manifest["models"])
    ...
    data_root = resolve_layer_path(config["output"]["data_dir"]) / "processed" / run_id
    checks = []
    ...
    for model in predictions:
        ...
        valid_targets = set(np.round(plan["target_exposure"].astype(float), 12)).issubset(
            {
                float(config["backtest"]["bear_exposure"]),
                float(config["backtest"]["neutral_exposure"]),
                float(config["backtest"]["target_exposure"]),
            }
        )
        check(
    ...
    if combined_ok:
        ...
        combined_ok = (
            ...
            and set(combined_signals["模型"])
            == {
                {"ridge": "Ridge", "cnn_gru": "CNN-GRU"}.get(model, model)
                for model in predictions
            }
        )
    ...
    write_json(log_dir / "backtest_validation_report.json", report)
