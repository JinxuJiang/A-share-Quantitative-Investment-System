...
def build_rebalance_plan(
    ...
    if not 0.0 <= bear_exposure <= neutral_exposure <= bull_exposure <= 1.0:
        raise ValueError(
            "状态仓位必须满足0 <= bear_exposure <= neutral_exposure "
            "<= target_exposure <= 1"
        )
    ...
    for date in dates:
        ...
        if prediction < threshold and zscore <= -z_threshold:
            desired_state, target_exposure = "bear", bear_exposure
...
def save_result(result: BacktestResult, data_root: Path, report_root: Path) -> None:
    model_data = Path(data_root) / result.model_name
    ...
    from run_storage import write_json

    write_json(model_report / "performance.json", result.metrics)
